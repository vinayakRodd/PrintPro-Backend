...
func (r *PrintJobRepository) GetByFilename(ctx context.Context, filename string) (*printjob.PrintJob, error) {
	...
	// Parse page_options JSONB (now includes crop_options)
...
// GetPendingByPartnerEmail retrieves all pending print jobs for a specific partner
// SECURITY: Only returns files where partner_email matches exactly and status = 'pending'
func (r *PrintJobRepository) GetPendingByPartnerEmail(ctx context.Context, partnerEmail string) ([]printjob.PrintJob, error) {
	rows, err := r.db.Query(ctx,
		`SELECT id, customer_email, partner_email, printer_id, filename, file_url, p_type, p_type as print_type, color, num_copies, page_options, back_to_back, delete_after_print, status, total_cost, created_at, updated_at
		 FROM print_jobs 
		 WHERE partner_email = $1 AND partner_email IS NOT NULL AND (status = 'pending' OR status IS NULL)
		 ORDER BY created_at ASC`,
		partnerEmail,
	)
	if err != nil {
		return nil, fmt.Errorf("failed to get pending print jobs by partner_email: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		// Only include jobs with status 'pending' or NULL (NULL is treated as pending)
		if job.Status != nil && *job.Status != printjob.StatusPending {
			continue
		}
...
func (r *PrintJobRepository) UpdatePrintOptions(ctx context.Context, filename string, partnerEmail string, accountEmail *string, color *bool, numCopies *int, startPage *int, endPage *int, pageFilterType *string, individualColorPages *[]int, skipPages *[]int, backToBack *bool, deleteAfterPrint *bool, printType *string, cropOptions *printjob.CropOptions) error {
	...
	// Check if any page-related fields or crop_options are being updated
	if startPage != nil || endPage != nil || pageFilterType != nil || individualColorPages != nil || skipPages != nil || cropOptions != nil {
		...
		// Include crop_options in page_options
		if cropOptions != nil {
			partialPageOpts["crop_options"] = cropOptions
		}
